...
from .forms import AnnouncementForm, AssignmentForm, ContactForm, CourseForm, MaterialForm
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    for field, errors in form.errors.items():
                        ...
                        field_errors_message = f"{field_label}: {', '.join(errors)}"
                        form_errors.append(field_errors_message)

                    form_errors_message = ', '.join(form_errors)
                    error_message = f'Error! Please correct the following form errors: {form_errors_message}'
                    messages.error(request, error_message)
                    return redirect('faculty_add_course')
            else:
                form = CourseForm()
            return render(request, 'main/faculty_add_course.html', {'form': form})
    else:
        return redirect('std_login')
...
def allMaterials(request, code):
    if is_faculty_authorised(request, code):
        ...
        return render(request, 'main/all-materials.html', context)
    else:
        return redirect('std_login')
...
def allMaterialsSTD(request, code):
    if is_student_authorised(request, code):
        course = Course.objects.get(code=code)
        materials = Material.objects.filter(course_code=course)
        context = {
        ...
        return render(request, 'main/all-materials-std.html', context)
    else:
        return redirect('std_login')
